...
interface TransitionEditorProps {
  ...
  onChange: (patch: { trigger?: SetlistTrigger; transition?: SetlistTransition }) => void;
  onClose: () => void;
}
 ...
 * (U9.3), which is the one thing about a setlist that surprises people — a
...
export function TransitionEditor({ step, isLast, anchor, onChange, onClose }: TransitionEditorProps) {
   ...
   *
   * `.setlist-track-strip` asks for `overflow-x: auto; overflow-y: visible`, and
  ...
  const setTrigger = (next: SetlistTrigger) => onChange({ trigger: next });
  const setTransition = (next: SetlistTransition) => onChange({ transition: next });
