...
/**
 * The playing half of chain mode.
 *
 ...
 * the number — Zen carries "step 3 → 120" there for a drill, and a chain step
...
function draw(over: Partial<Parameters<typeof ChainPlayer>[0]> = {}) {
  return render(
    <ChainPlayer
      chain={CHAIN}
      step={CHAIN.steps[0]}
...
describe("the chain, playing", () => {
  it("leads with the step's name and its tempo, nothing else", () => {
    const { container } = draw();
    expect(container.querySelector(".chain-player-name")!.textContent).toBe("Loosen up");
    expect(container.querySelector(".chain-player-bpm")!.textContent).toBe("70");
    expect(container.querySelector(".chain-player-where")!.textContent).toContain("Step 1 of 3");
    ...
    expect(screen.getByRole("button").textContent).toBe("Edit the chain");
  });

  it("counts bars for a step set in bars", () => {
    const { container } = draw({ remaining: { kind: "bars", bars: 6 } });
    expect(container.querySelector(".chain-player-left")!.textContent).toBe("bar 3 of 8");
  });
  ...
  it("counts a step set in minutes in BOTH — the time left and the bar", () => {
    ...
    const left = container.querySelector(".chain-player-left")!.textContent!;
  ...
  it("says a step nobody can time is waiting for you", () => {
    ...
    expect(container.querySelector(".chain-player-left")!.textContent).toBe("waiting for you");
  });
  ...
  it("draws the routine as a bar, each step as wide as it is long", () => {
    ...
    const { container } = draw();
    const segs = [...container.querySelectorAll<HTMLElement>(".chain-player-seg")];
    ...
    expect([...container.querySelectorAll(".chain-player-seg-fill")]).toHaveLength(1);
  });
  ...
  it("names what is coming, and says how only when there is something to say", () => {
    ...
    const plain = draw().container.querySelector(".chain-player-next")!.textContent!;
    ...
    const counted = draw({ step: CHAIN.steps[1], stepNumber: 2 }).container
      .querySelector(".chain-player-next")!.textContent!;
  ...
  it("draws a dot per beat, grouped the way the step is grouped", () => {
    ...
    expect(container.querySelectorAll(".chain-player-group")).toHaveLength(3);
    expect(container.querySelectorAll(".chain-player-dot")).toHaveLength(7);
  ...
  it("says the chain is about to end rather than naming a step that is not there", () => {
    ...
    expect(container.querySelector(".chain-player-next")!.textContent).toBe("Last step");
  });
});
