...
import type { ChainStep, ChainTransition, ChainTrigger, Subdivision } from "../../types";

/**
 * One step of a chain, written out as a sentence you can reach into.
 *
...
interface StepSentenceProps {
  ...
  total: number;
  isLast: boolean;
  /** The whole step, patched. The paragraph owns the chain. */
  onChange: (patch: Partial<Omit<ChainStep, "id">>) => void;
...
export function StepSentence({
  ...
  const note = t("chain.said.stepNote", { number, total });

  const gapPatch = (patch: { trigger?: ChainTrigger; transition?: ChainTransition }) =>
    onChange(patch);

  return (
    <div className={`drill-plan-block chain-sentence${quiet ? " chain-sentence-quiet" : ""}`}>
      ...
      {/* The line the cards never had room for, and the one a chain is
      ...
      <div className="drill-plan chain-plan-timing">
        <span className="chain-plan-glue">
          {step.trigger.kind === "manual" ? t("chain.said.runs") : t("chain.said.for")}
        </span>
        <button
          type="button"
          ref={anchor("gap")}
          ...
          onClick={toggle("gap")}
        >
          <span className="drill-plan-value chain-plan-mid">{triggerLabel(t, step.trigger)}</span>
        </button>
        <span className="chain-plan-glue">
          {isLast ? t("chain.said.andThen") : t("chain.said.then")}
        </span>
        <button
          type="button"
          className={`drill-plan-token${open === "gap" ? " open" : ""}`}
          ...
          onClick={toggle("gap")}
        >
          <span className="drill-plan-value chain-plan-mid">
            {isLast ? t("chain.said.theChainEnds") : transitionLabel(t, step.transition)}
          </span>
        </button>
      </div>

      <div className="drill-plan-detail chain-plan-detail">
        <button
          type="button"
          ref={anchor("name")}
        ...
        >
          {t("chain.said.volume", { percent: Math.round(step.volume * 100) })}
        </button>
      </div>
      ...
      {open === "meter" && (
        ...
        >
          ...
          <DrillPopoverRow label={t("drill.beats")} tip={t("chain.said.beatsTip")}>
            <DrillNumberField
              value={beats || 4}
              min={1}
              max={12}
      ...
      {/* The click, and here it is the STEP's, not the app's. A chain that
          could not change its sound between steps would be a chain that
      ...
      {open === "volume" && (
        <DrillConfigPopover
          ...
          onClose={close}
          clears=".chain-sentence"
          label={t("chain.said.volumeLabel")}
          note={note}
        >
          <DrillPopoverRow label={t("chain.said.volumeLabel")}>
            <input
              className="chain-volume-range"
              type="range"
              min={0}
              max={100}
              value={Math.round(step.volume * 100)}
              aria-label={t("chain.said.volumeLabel")}
      ...
      {open === "name" && (
        <DrillConfigPopover
          ...
          onClose={close}
          clears=".chain-sentence"
          label={t("chain.renameStep")}
        >
          <DrillPopoverRow label={t("chain.renameStep")}>
            <input
              className="chain-name-input"
              value={nameDraft}
              maxLength={24}
              autoFocus
              aria-label={t("chain.renameStep")}
