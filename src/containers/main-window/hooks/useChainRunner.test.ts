 ...
 * setters, and that a finished chain stops the transport.
 */
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { act, renderHook } from "@testing-library/react";
import { useChainRunner } from "./useChainRunner";
import { mockInvoke } from "../../../test/mocks";
import type { BeatEvent, Chain, ChainStep, ChainTrigger } from "../../../types";

function step(name: string, bpm: number, trigger: ChainTrigger): ChainStep {
  return {
    id: `s-${name}`,
    name,
    bpm,
...
describe("useChainRunner", () => {
  ...
  it("does nothing at all without a chain", () => {
  ...
  it("stops the transport when the chain runs out (U9.6)", () => {
    const single: Chain = { ...CHAIN, steps: [CHAIN.steps[0]] };
    const { result, rerender } = renderHook(
      ({ b, playing }: { b: BeatEvent | null; playing: boolean }) =>
        useChainRunner(single, playing, b),
      { initialProps: { b: null as BeatEvent | null, playing: false } },
    );
    act(() => rerender({ b: null, playing: true }));
    act(() => rerender({ b: beat(0, true), playing: true }));
  ...
  it("counts a seconds gap down between beats (U9.7)", () => {
    ...
    const { result, rerender } = renderHook(
      ({ b, playing }: { b: BeatEvent | null; playing: boolean }) =>
        useChainRunner(timed, playing, b),
      { initialProps: { b: null as BeatEvent | null, playing: false } },
    );
    act(() => rerender({ b: null, playing: true }));
    act(() => rerender({ b: beat(0, true), playing: true }));
  ...
  it("puts the volume back if the run is stopped inside a rest", () => {
    ...
    const { rerender } = renderHook(
      ({ b, playing }: { b: BeatEvent | null; playing: boolean }) =>
        useChainRunner(resting, playing, b),
      { initialProps: { b: null as BeatEvent | null, playing: false } },
    );
    act(() => rerender({ b: null, playing: true }));
    act(() => rerender({ b: beat(0, true), playing: true }));
