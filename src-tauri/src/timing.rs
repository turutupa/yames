...
// `BeatTick` / `BeatLog` moved to their own pure-Rust module so `engine.rs`
// can log beats in a mobile build without compiling the scoring pipeline
// (M01). Re-exported so every existing `timing::BeatTick` path resolves.
pub use crate::beat_log::{create_beat_log, BeatLog, BeatTick};
