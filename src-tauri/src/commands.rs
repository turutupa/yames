...
// `Manager` was desktop-only while the only things reaching for it were the
// windows and the app's paths (M01). The band needs it on every platform:
// `set_jam` and `warm_jam` pull the kit, voice and gain caches out of managed
// state, and the band plays on a phone (M08).
use tauri::Manager;
...
///
/// Desktop only. A phone has no folder of samples to point at and no file
/// manager to point at it with, so the custom kit is one of the two things
/// Jam leaves behind on the way to a phone (M08); `tauri-plugin-dialog`
/// leaves the mobile dependency graph with it.
#[cfg(desktop)]
#[tauri::command]
...
/// not repainting.
///
/// Desktop only, with `pick_kit_folder`: there is nothing to inspect on a
/// phone.
#[cfg(desktop)]
#[tauri::command]
...
///
/// Desktop only, with the commands below it: a take mixes the MIC in, and
/// `audio_input` is not part of a mobile build (M08).
#[cfg(desktop)]
#[derive(Default)]
