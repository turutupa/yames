...
// `TempoContext` moved to `tempo_context.rs` so the mobile build can use the
// engine's three atomics without compiling aubio (M01). The detector still
// reads it here; every other caller now imports `crate::tempo_context`
