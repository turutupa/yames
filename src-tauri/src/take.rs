...
///
/// Jam ids are generated by the UI and are already tame, but they arrive
/// over IPC and a directory name built from unvalidated input is how a
/// feature that writes files becomes a feature that writes ANY file. Only
...
/// A take id as a file stem. Same rule, and the ids this module generates
/// are digits, so anything else came from somewhere it should not have.
fn safe_stem(id: &str) -> Result<String, String> {
    let s = safe_dir_name(id)?;
    if s != id.trim() {
...
fn duration_of(path: &Path) -> f64 {
    ...
    (bytes.len() - 44) as f64 / 2.0 / sr as f64
}
...
mod tests {
    ...
    fn a_jam_id_cannot_write_outside_the_takes_directory() {
        ...
        assert_eq!(safe_dir_name("jam-17a3f").unwrap(), "jam-17a3f");
        assert_eq!(safe_dir_name("../../etc").unwrap(), ".._.._etc");
    ...
    fn write_take(root: &Path, jam: &str, id: &str, secs: f64) -> PathBuf {
        let dir = root.join(TAKES_DIR).join(jam);
