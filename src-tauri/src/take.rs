...
//!
//! ## Compiled everywhere, reachable on a desktop
//!
//! Nothing here runs on a phone: the commands that start, stop, list and
//! play a take are `cfg(desktop)`, because a take mixes the MIC in and the
//! mic is not part of a mobile build (M08). The module itself still
//! compiles there, because the output callback reads its ring and the one
//! thing this port never does is give the click a second shape — so the
//! slot is present and permanently empty. Hence the blanket `dead_code`
//! allowance on a mobile build, and only there.
...
///
/// Pulled out of `SessionAudioRecorder::finish` in `session_audio.rs` so this file and the diagnostic dump write byte
...
///
/// It lives here rather than beside that recorder because this module
/// compiles on every platform and `session_audio` does not — the mic dump is
/// desktop-only, and a take's file format is not. It writes no file and
/// records nothing: it lays out forty-four bytes.
...
impl TakeWavWriter {
    ...
    fn finish(mut self) -> std::io::Result<(PathBuf, u64)> {
        ...
        let header = wav_header_mono_16bit(self.sample_rate, self.samples);
...
mod tests {
    ...
    fn a_file_that_is_not_one_of_ours_is_refused_rather_than_guessed_at() {
        ...
        let mut stereo = wav_header_mono_16bit(48_000, 2).to_vec();
    ...
    fn the_length_of_a_take_comes_from_its_size_and_its_header() {
        let h = wav_header_mono_16bit(48_000, 0);
        ...
        let h44 = wav_header_mono_16bit(44_100, 0);
