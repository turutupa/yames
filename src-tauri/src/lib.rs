...
mod engine;
pub mod instrument;
/// The band. Both halves ship on a phone: `jam` compiles the table the
/// callback reads, `kit` decodes the sound bank it reads samples out of.
/// Neither pulls a desktop-only crate — see the `hound` / `claxon` note in
/// `Cargo.toml`.
mod jam;
mod kit;
mod state;
mod tempo_context;
/// Where a take's audio and the coach's speech wait for the audio callback.
///
/// Both are compiled on every platform on purpose. They are pure std +
/// serde, and they are the two slots the output callback reads on `main`:
/// gating them off a phone would mean two versions of the callback body,
/// and the one thing this port never does is give the click a second
/// shape. On a phone nothing ever fills either slot — the mic
/// (`audio_input`) and the voice (`tts`) are both `cfg(desktop)`, and so
/// are the commands that would start a take or say a line — so the
/// callback's relaxed load finds an empty slot every buffer, exactly as it
/// does on a desktop with the coach silent.
mod speech_out;
mod take;
...
#[cfg(desktop)]
use commands::{DownloadState, TakeState};
...
pub fn run() {
    ...
    let builder = builder.setup(move |app| {
            ...
            // Desktop only: a take is your playing mixed with the band, and
            // the mic it needs is not compiled into a phone build (M08).
            #[cfg(desktop)]
