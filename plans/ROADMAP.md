...
| **LLM in releases** | Fixed 2026-09-02 (T01/T02): every release platform compiles the backend and CI smoke-tests a real GGUF. Historical note: before that day,  `default = []`, and `.github/workflows/release.yml` passes no `--features`. Every shipped binary is template-only. `load_model` returns `Ok(true)` without the feature, so the UI reports the brain as loaded after the user downloads 1.1–2.4 GB of weights that are never read. Enabling the feature as-is breaks Windows/Linux (`metal`). |
